...
│
├── code/
│   ├── 00_audit_5_datasets_visual.py
│   ├── 03_generate_dataset_a.py
│   ├── 04_add_2_llm_fields.py
│   ├── 08_pulmo_llm_all_in_one.py
│   ├── 09_compare_base_vs_finetuned_all.py
│   ├── 14_compare_qwen_llava_melspec_lungsound_study.py
│   └── audit_pulmo_datasets.py
│
├── configs/
│   ├── dataset_audit_environment.yml
│   └── simple_report_environment.yml
│
├── docs/
│   ├── workflow.md
...
│
├── results_examples/
│   ├── sample_dataset_summary.csv
│   ├── sample_report_metrics.csv
│   └── sample_graphs/
│
└── figures/
    ├── workflow_flowchart.png
    └── model_comparison_diagram.png
```

Do not upload full datasets, model weights, adapters, checkpoints, `.wav` files, or private clinical data to GitHub. Upload only code, configuration files, documentation, and small anonymized sample outputs.
...
Only upload it if your repository also includes the local VLM comparison experiment.
...
```text
D:\LungSound\ICBHI\Dataset
D:\LungSound\Mendley lung\Audio Files
D:\LungSound\HLS-CMDS\Dataset
D:\LungSound\Models
D:\LungSound\Pulmo-LLM\Results\Pulmo_LLM_Qwen\adapter
checkpoint-* folders
...
```bat
mkdir D:\GitHub\Pulmo-LLM
cd /d D:\GitHub\Pulmo-LLM
```
...
```bat
mkdir code configs docs figures results_examples
copy D:\LungSound\Pulmo-LLM\Code\*.py code\
copy D:\LungSound\Pulmo-LLM\Code\*.yml configs\
copy D:\LungSound\Pulmo-LLM\Code\README.md README.md
```
...
```bat
git add .
git commit -m "Initial Pulmo-LLM benchmark pipeline"
```

Create a GitHub repository named:

```text
Pulmo-LLM-Lung-Sound-Report-Benchmark
```
...
```bat
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/Pulmo-LLM-Lung-Sound-Report-Benchmark.git
...
```text
Benchmarking a general-purpose Qwen LLM against a domain-adapted Pulmo-LLM for lung sound report generation using ICBHI, Mendeley Lung Sounds, and HLS-CMDS datasets.
```
